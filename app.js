...
function completedRow(item,s){
  ...
  if(item.type==="singleCourse"){
    ...
    item.options.forEach(code=>{
      ...
      const row=el(`<div class="course-row">
        <div class="course-meta"><span class="badge">${c.code}</span>
      ...
      cb.onchange=()=>{
        ...
        if(cb.checked){ item.options.forEach(other=>{ if(other!==c.code) set.delete(other); }); set.add(c.code); }
        else { set.delete(c.code); }
        st.taken=[...set]; Store.write(st);
        $$(".acc-body input",sub).forEach(inp=>{ const code=inp.getAttribute("data-code"); inp.checked=st.taken.includes(code); });
      };
      body.append(row);
    });
    wrap.append(sub);
